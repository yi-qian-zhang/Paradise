...
import com.mygdx.monster.round_2.*;
...
/**
 * This Screen represents map 2-2
 * @author Yang Tang
 * @version 1.0
 */
public class Screen2_2 extends LevelScreen {
    static {
        fileName = "maps/map_2/Map 2-2.tmx";
        nextScreenClass = "com.mygdx.screens.Screen2_3";
        thisScreenClass = "com.mygdx.screens.Screen2_2";
    }
    ...
    public void initialize() {
        ...
        for (MapObject obj : tma.getTileList("Mon16")) {
            MapProperties props = obj.getProperties();
            String[] files = {"monster/round_2/monster_2/2-1.png", "monster/round_2/monster_2/2-2.png"};
            new Wartortle((float) props.get("x"), (float) props.get("y"), mainStage, files);
        }
        ...
        for (MapObject obj : tma.getTileList("Mon17")) {
            MapProperties props = obj.getProperties();
            String[] files = {"monster/round_2/monster_5/5-1.png", "monster/round_2/monster_5/5-2.png"};
            new Golduck((float) props.get("x"), (float) props.get("y"), mainStage, files);
        }

        for (MapObject obj : tma.getTileList("Mon18")) {
            MapProperties props = obj.getProperties();
            new Poliwag((float) props.get("x"), (float) props.get("y"), mainStage, "monster/round_2/monster_6/6-1.png");
        }
    ...
    public void update(float dt) {
        ...
        for (BaseActor end1 : BaseActor.getList(mainStage, "com.mygdx.maps.End1")) {
            if (jack.overlaps(end1)) {
                start = "Start1";
                ParadiseGame.setActiveScreen(new Screen2_7());
            }
        }
        ...
        for (BaseActor tip : BaseActor.getList(mainStage, "com.mygdx.maps.Tip1")) {
            if (jack.overlaps(tip) && Gdx.input.isKeyPressed(Input.Keys.Q)) {
                new TipActor(Tip1.x, Tip1.y, mainStage, "items/tips/map_2/tip3.png");
                tip.remove();
            }
        }
        ...
        for (BaseActor monster : BaseActor.getList(mainStage, "com.mygdx.monster.round_2.Wartortle")) {
        ...
        for (BaseActor monster : BaseActor.getList(mainStage, "com.mygdx.monster.round_2.Golduck")) {
            if (jack.overlaps(monster)) {
                ParadiseGame.backgroundMusic.pause();
                if (isDeathSoundPermitted) {
                    ParadiseGame.deathSound.play();
