...
import com.mygdx.monster.round_2.Starmie;
import com.mygdx.monster.round_2.Staryu;
...
/**
 * This Screen represents map 2-6
 * @author Yang Tang
 * @version 1.0
 */
public class Screen2_6 extends LevelScreen {

    static {
        fileName = "maps/map_2/Map 2-6.tmx";
        thisScreenClass = "com.mygdx.screens.Screen2_6";
    }
    ...
    public void initialize() {
        ...
        for (MapObject obj : tma.getTileList("Mon23")) {
            MapProperties props = obj.getProperties();
            new Staryu((float) props.get("x"), (float) props.get("y"), mainStage, "monster/round_2/monster_19/19-1.png");
        }

        for (MapObject obj : tma.getTileList("Mon24")) {
            MapProperties props = obj.getProperties();
            new Starmie((float) props.get("x"), (float) props.get("y"), mainStage, "monster/round_2/monster_20/20-1.png");
        }
    ...
    public void update(float dt) {
        ...
        for (BaseActor tip : BaseActor.getList(mainStage, "com.mygdx.maps.Tip1")) {
            if (jack.overlaps(tip) && Gdx.input.isKeyPressed(Input.Keys.Q)) {
                new TipActor(Tip1.x, Tip1.y, mainStage, "items/tips/map_2/tip9.png");
                tip.remove();
                ParadiseGame.setActiveScreen(new Screen2_2());
            }
        }

        for (BaseActor tip : BaseActor.getList(mainStage, "com.mygdx.maps.Tip2")) {
            if (jack.overlaps(tip) && Gdx.input.isKeyPressed(Input.Keys.Q)) {
                new TipActor(Tip2.x, Tip2.y, mainStage, "items/tips/map_2/tip10.png");
                tip.remove();
            }
        }
        ...
        for (BaseActor tip : BaseActor.getList(mainStage, "com.mygdx.maps.Tip3")) {
            if (jack.overlaps(tip) && Gdx.input.isKeyPressed(Input.Keys.Q)) {
                new TipActor(Tip3.x, Tip3.y, mainStage, "items/tips/map_2/tip11.png");
                tip.remove();
            }
        }
        ...
        for (BaseActor monster : BaseActor.getList(mainStage, "com.mygdx.monster.round_2.Staryu")) {
        ...
        for (BaseActor monster : BaseActor.getList(mainStage, "com.mygdx.monster.round_2.Starmie")) {
            if (jack.overlaps(monster)) {
                ParadiseGame.backgroundMusic.pause();
                if (isDeathSoundPermitted) {
                    ParadiseGame.deathSound.play();
