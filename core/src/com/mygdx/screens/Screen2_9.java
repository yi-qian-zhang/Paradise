...
/**
 * This Screen represents map 2-9
 * @author Yang Tang
 * @version 1.0
 */
public class Screen2_9 extends LevelScreen {

    static {
        fileName = "maps/map_2/Map 2-9.tmx";
        thisScreenClass = "com.mygdx.screens.Screen2_9";
    }
    ...
    public void initialize() {
        ...
        for (MapObject obj : tma.getTileList("Mon27")) {
            MapProperties props = obj.getProperties();
            new Seel((float) props.get("x"), (float) props.get("y"), mainStage, "monster/round_2/monster_13/13-1.png");
        }

        for (MapObject obj : tma.getTileList("Mon28")) {
            MapProperties props = obj.getProperties();
            new Dewgong((float) props.get("x"), (float) props.get("y"), mainStage, "monster/round_2/monster_14/14-1.png");
        }

        for (MapObject obj : tma.getTileList("Mon29")) {
            MapProperties props = obj.getProperties();
            new Horsea((float) props.get("x"), (float) props.get("y"), mainStage, "monster/round_2/monster_15/15-1.png");
        }

        for (MapObject obj : tma.getTileList("Mon30")) {
            MapProperties props = obj.getProperties();
            new Seadra((float) props.get("x"), (float) props.get("y"), mainStage, "monster/round_2/monster_16/16-1.png");
        }
    ...
    public void update(float dt) {
        ...
        for (BaseActor end1 : BaseActor.getList(mainStage, "com.mygdx.maps.End1"))
            if (jack.overlaps(end1)) {
               start = "Start1";
                ParadiseGame.setActiveScreen(new Screen2_9());
            }
        ...
        for (BaseActor tip : BaseActor.getList(mainStage, "com.mygdx.maps.Tip1")) {
            if (jack.overlaps(tip) && Gdx.input.isKeyPressed(Input.Keys.Q)) {
                new TipActor(Tip1.x, Tip1.y, mainStage, "items/tips/map_2/tip15.png");
                tip.remove();
                ParadiseGame.setActiveScreen(new Screen2_3());
            }
        }
