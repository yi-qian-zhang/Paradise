...
import com.mygdx.monster.round_3.Beedrill;
...
/**
 * This Screen represents map 3-2
 * @author Yang Tang
 * @version 1.0
 */
public class Screen3_2 extends LevelScreen {
    static {
        fileName = "maps/map_3/Map 3-2.tmx";
        nextScreenClass = "com.mygdx.screens.Screen3_3";
        thisScreenClass = "com.mygdx.screens.Screen3_2";
    }
    ...
    public void initialize() {
        ...
        for (MapObject obj : tma.getTileList("Mon35")) {
            MapProperties props = obj.getProperties();
            new Butterfree((float) props.get("x"), (float) props.get("y"), mainStage, "monster/round_3/monster_1/1-1.png");
        }

        for (MapObject obj : tma.getTileList("Mon36")) {
            MapProperties props = obj.getProperties();
            new Beedrill((float) props.get("x"), (float) props.get("y"), mainStage, "monster/round_3/monster_2/2-1.png");
        }
    ...
    public void update(float dt) {
        ...
        for (BaseActor monster : BaseActor.getList(mainStage, "com.mygdx.monster.round_3.Butterfree")) {
        ...
        for (BaseActor monster : BaseActor.getList(mainStage, "com.mygdx.monster.round_3.Beedrill")) {
            if (jack.overlaps(monster)) {
                ParadiseGame.backgroundMusic.pause();
                if (isDeathSoundPermitted) {
                    ParadiseGame.deathSound.play();
        ...
        for (BaseActor tip : BaseActor.getList(mainStage, "com.mygdx.maps.Tip1")) {
            if (jack.overlaps(tip) && Gdx.input.isKeyPressed(Input.Keys.Q)) {
                new TipActor(Tip1.x, Tip1.y, mainStage, "items/tips/map_3/tip2.png");
                tip.remove();
            }
        }
