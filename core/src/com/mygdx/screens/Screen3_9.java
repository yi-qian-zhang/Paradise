...
import com.mygdx.monster.round_3.Fearow;
import com.mygdx.monster.round_3.Pidgertto;
...
/**
 * This Screen represents map 3-9
 * @author Yang Tang
 * @version 1.0
 */
public class Screen3_9 extends LevelScreen {
    static {
        fileName = "maps/map_3/Map 3-9.tmx";
        thisScreenClass = "com.mygdx.screens.Screen3_9";
    }
    ...
    public void initialize() {
        ...
        for (MapObject obj : tma.getTileList("Mon46")) {
            MapProperties props = obj.getProperties();
            new Pidgertto((float) props.get("x"), (float) props.get("y"), mainStage, "monster/round_3/monster_5/5-1.png");
        }
        ...
        for (MapObject obj : tma.getTileList("Mon47")) {
            MapProperties props = obj.getProperties();
            new Fearow((float) props.get("x"), (float) props.get("y"), mainStage, "monster/round_3/monster_7/7-1.png");
        }
    ...
    public void update(float dt) {
        ...
        for (BaseActor end1 : BaseActor.getList(mainStage, "com.mygdx.maps.End1")) {
            if (jack.overlaps(end1) && Gdx.input.isKeyPressed(Input.Keys.Q)) {
                ParadiseGame.setActiveScreen(new Screen4_1());
            }
        }
        ...
        for (BaseActor monster : BaseActor.getList(mainStage, "com.mygdx.monster.round_3.Pidgertto")) {
        ...
        for (BaseActor monster : BaseActor.getList(mainStage, "com.mygdx.monster.round_3.Fearow")) {
            if (jack.overlaps(monster)) {
                ParadiseGame.backgroundMusic.pause();
                if (isDeathSoundPermitted) {
                    ParadiseGame.deathSound.play();
        ...
        for (BaseActor tip : BaseActor.getList(mainStage, "com.mygdx.maps.Tip1")) {
            if (jack.overlaps(tip) && Gdx.input.isKeyPressed(Input.Keys.Q)) {
                new TipActor(Tip1.x - 300, Tip1.y - 150, mainStage, "items/tips/map_3/tip12.png");
                tip.remove();
            }
        }
