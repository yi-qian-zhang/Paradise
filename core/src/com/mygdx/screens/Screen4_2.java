...
import com.mygdx.monster.round_4.Diglett;
import com.mygdx.monster.round_4.Sandshrew;
...
/**
 * This Screen represents map 4-2
 * @author Yang Tang
 * @version 1.0
 */
public class Screen4_2 extends LevelScreen {
    static {
        fileName = "maps/map_4/Map 4-2.tmx";
        nextScreenClass = "com.mygdx.screens.Screen4_3";
        thisScreenClass = "com.mygdx.screens.Screen4_2";
    }
    ...
    public void initialize() {
        ...
        for (MapObject obj : tma.getTileList("Mon50")) {
            MapProperties props = obj.getProperties();
            new Diglett((float) props.get("x"), (float) props.get("y"), mainStage, "monster/round_4/monster_8/8-1.png");
        }

        for (MapObject obj : tma.getTileList("Mon51")) {
            MapProperties props = obj.getProperties();
            new Sandshrew((float) props.get("x"), (float) props.get("y"), mainStage, "monster/round_4/monster_4/4-1.png");
        }
    ...
    public void update(float dt) {
        ...
        for (BaseActor end1 : BaseActor.getList(mainStage, "com.mygdx.maps.End1")) {
            if (jack.overlaps(end1)) {
                start = "Start";
                ParadiseGame.setActiveScreen(new Screen4_6());
            }
        }
        ...
        for (BaseActor monster : BaseActor.getList(mainStage, "com.mygdx.monster.round_4.Diglett")) {
        ...
        for (BaseActor monster : BaseActor.getList(mainStage, "com.mygdx.monster.round_4.Sandshrew")) {
            if (jack.overlaps(monster)) {
                ParadiseGame.backgroundMusic.pause();
                if (isDeathSoundPermitted) {
                    ParadiseGame.deathSound.play();
        ...
        for (BaseActor tip : BaseActor.getList(mainStage, "com.mygdx.maps.Tip1")) {
            if (jack.overlaps(tip) && Gdx.input.isKeyPressed(Input.Keys.Q)) {
                new TipActor(Tip1.x, Tip1.y, mainStage, "items/tips/map_4/tip3.png");
                tip.remove();
            }
        }
        ...
        for (BaseActor tip : BaseActor.getList(mainStage, "com.mygdx.maps.Tip2")) {
            if (jack.overlaps(tip) && Gdx.input.isKeyPressed(Input.Keys.Q)) {
                new TipActor(Tip2.x - 300, Tip2.y, mainStage, "items/tips/map_4/tip4.png");
                tip.remove();
            }
        }
