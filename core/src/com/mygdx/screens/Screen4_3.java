...
import com.mygdx.monster.round_4.Ivysaur;
import com.mygdx.monster.round_4.Weepinbell;
...
/**
 * This Screen represents map 4-3
 * @author Yang Tang
 * @version 1.0
 */
public class Screen4_3 extends LevelScreen {
    static {
        fileName = "maps/map_4/Map 4-3.tmx";
        nextScreenClass = "com.mygdx.screens.Screen4_4";
        thisScreenClass = "com.mygdx.screens.Screen4_3";
    }
    ...
    public void initialize() {
        ...
        for (MapObject obj : tma.getTileList("Mon52")) {
            MapProperties props = obj.getProperties();
            new Ivysaur((float) props.get("x"), (float) props.get("y"), mainStage, "monster/round_4/monster_2/2-1.png");
        }

        for (MapObject obj : tma.getTileList("Mon53")) {
            MapProperties props = obj.getProperties();
            new Weepinbell((float) props.get("x"), (float) props.get("y"), mainStage, "monster/round_4/monster_11/11-1.png");
        }
    ...
    public void update(float dt) {
        ...
        for (BaseActor end1 : BaseActor.getList(mainStage, "com.mygdx.maps.End1")) {
            if (jack.overlaps(end1)) {
                start = "Start";
                ParadiseGame.setActiveScreen(new Screen4_7());
            }
        }
        ...
        for (BaseActor monster : BaseActor.getList(mainStage, "com.mygdx.monster.round_4.Ivysaur")) {
        ...
        for (BaseActor monster : BaseActor.getList(mainStage, "com.mygdx.monster.round_4.Weepinbell")) {
            if (jack.overlaps(monster)) {
                ParadiseGame.backgroundMusic.pause();
                if (isDeathSoundPermitted) {
                    ParadiseGame.deathSound.play();
        ...
        for (BaseActor tip : BaseActor.getList(mainStage, "com.mygdx.maps.Tip1")) {
            if (jack.overlaps(tip) && Gdx.input.isKeyPressed(Input.Keys.Q)) {
                new TipActor(Tip1.x, Tip1.y, mainStage, "items/tips/map_4/tip5.png");
                tip.remove();
            }
        }
        ...
        for (BaseActor tip : BaseActor.getList(mainStage, "com.mygdx.maps.Tip2")) {
            if (jack.overlaps(tip) && Gdx.input.isKeyPressed(Input.Keys.Q)) {
                new TipActor(Tip2.x, Tip2.y - 100, mainStage, "items/tips/map_4/tip6.png");
                tip.remove();
            }
        }
