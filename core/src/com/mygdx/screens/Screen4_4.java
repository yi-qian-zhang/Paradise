...
import com.mygdx.monster.round_4.Cubone;
import com.mygdx.monster.round_4.Venusaur;
import com.mygdx.monster.round_4.Victreebel;
...
/**
 * This Screen represents map 4-4
 * @author Yang Tang
 * @version 1.0
 */
public class Screen4_4 extends LevelScreen {
    static {
        fileName = "maps/map_4/Map 4-4.tmx";
        thisScreenClass = "com.mygdx.screens.Screen4_4";
    }
    ...
    public void initialize() {
        ...
        for (MapObject obj : tma.getTileList("DigPower")) {
            MapProperties props = obj.getProperties();
            new DigPower((float) props.get("x"), (float) props.get("y"), mainStage);
        }
        ...
        for (MapObject obj : tma.getTileList("Mon54")) {
            MapProperties props = obj.getProperties();
            new Venusaur((float) props.get("x"), (float) props.get("y"), mainStage, "monster/round_4/monster_3/3-1.png");
        }

        for (MapObject obj : tma.getTileList("Mon55")) {
            MapProperties props = obj.getProperties();
            new Victreebel((float) props.get("x"), (float) props.get("y"), mainStage, "monster/round_4/monster_12/12-1.png");
        }

        for (MapObject obj : tma.getTileList("Mon56")) {
            MapProperties props = obj.getProperties();
            new Cubone((float) props.get("x"), (float) props.get("y"), mainStage, new String[] {"monster/round_4/monster_17/17-1.png", "monster/round_4/monster_17/17-2.png"});
        }
    ...
    public void update(float dt) {
        ...
        for (BaseActor end1 : BaseActor.getList(mainStage, "com.mygdx.maps.End1")) {
            if (jack.overlaps(end1)) {
                start = "Start";
                ParadiseGame.setActiveScreen(new Screen4_8());
            }
        }
        ...
        for (BaseActor monster : BaseActor.getList(mainStage, "com.mygdx.monster.round_4.Venusaur")) {
        ...
        for (BaseActor monster : BaseActor.getList(mainStage, "com.mygdx.monster.round_4.Victreebel")) {
            if (jack.overlaps(monster)) {
                ParadiseGame.backgroundMusic.pause();
                if (isDeathSoundPermitted) {
                    ParadiseGame.deathSound.play();
        ...
        for (BaseActor monster : BaseActor.getList(mainStage, "com.mygdx.monster.round_4.Cubone")) {
            if (jack.overlaps(monster)) {
                ParadiseGame.backgroundMusic.pause();
                if (isDeathSoundPermitted) {
                    ParadiseGame.deathSound.play();
        ...
        for (BaseActor monster : BaseActor.getList(mainStage, "com.mygdx.monster.round_4.CuboneBone")) {
            if (jack.overlaps(monster)) {
                ParadiseGame.backgroundMusic.pause();
                if (isDeathSoundPermitted) {
                    ParadiseGame.deathSound.play();
        ...
        for (BaseActor power : BaseActor.getList(mainStage, "com.mygdx.maps.DigPower"))
        ...
        for (BaseActor tip : BaseActor.getList(mainStage, "com.mygdx.maps.Tip1")) {
            if (jack.overlaps(tip) && Gdx.input.isKeyPressed(Input.Keys.Q)) {
                new TipActor(Tip1.x, Tip1.y, mainStage, "items/tips/map_4/tip7.png");
                tip.remove();
            }
        }
        ...
        for (BaseActor tip : BaseActor.getList(mainStage, "com.mygdx.maps.Tip2")) {
            if (jack.overlaps(tip) && Gdx.input.isKeyPressed(Input.Keys.Q)) {
                new TipActor(Tip2.x, Tip2.y, mainStage, "items/tips/map_4/tip8.png");
                tip.remove();
            }
        }
