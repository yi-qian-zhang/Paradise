...
import com.mygdx.monster.round_4.Dugrio;
import com.mygdx.monster.round_4.Sandslash;
...
/**
 * This Screen represents map 4-6
 * @author Yang Tang
 * @version 1.0
 */
public class Screen4_6 extends LevelScreen {
    static {
        fileName = "maps/map_4/Map 4-6.tmx";
        thisScreenClass = "com.mygdx.screens.Screen4_6";
    }
    ...
    public void initialize() {
        ...
        for (MapObject obj : tma.getTileList("Mon59")) {
            MapProperties props = obj.getProperties();
            new Sandslash((float) props.get("x"), (float) props.get("y"), mainStage, "monster/round_4/monster_5/5-1.png");
        }

        for (MapObject obj : tma.getTileList("Mon60")) {
            MapProperties props = obj.getProperties();
            new Dugrio((float) props.get("x"), (float) props.get("y"), mainStage, "monster/round_4/monster_9/9-1.png");
        }
    ...
    public void update(float dt) {
        ...
        for (BaseActor end1 : BaseActor.getList(mainStage, "com.mygdx.maps.End1")) {
            if (jack.overlaps(end1)) {
                start = "Start";
                ParadiseGame.setActiveScreen(new Screen4_9());
            }
        }
        ...
        for (BaseActor monster : BaseActor.getList(mainStage, "com.mygdx.monster.round_4.Sandslash")) {
        ...
        for (BaseActor monster : BaseActor.getList(mainStage, "com.mygdx.monster.round_4.Dugrio")) {
            if (jack.overlaps(monster)) {
                ParadiseGame.backgroundMusic.pause();
                if (isDeathSoundPermitted) {
                    ParadiseGame.deathSound.play();
        ...
        for (BaseActor tip : BaseActor.getList(mainStage, "com.mygdx.maps.Tip1")) {
            if (jack.overlaps(tip) && Gdx.input.isKeyPressed(Input.Keys.Q)) {
                new TipActor(Tip1.x, Tip1.y, mainStage, "items/tips/map_4/tip10.png");
                tip.remove();
            }
        }
