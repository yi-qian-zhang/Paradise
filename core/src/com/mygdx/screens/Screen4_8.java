...
import com.mygdx.monster.round_4.Onix;
import com.mygdx.monster.round_4.Rhydon;
import com.mygdx.monster.round_4.Rhyhorn;
...
/**
 * This Screen represents map 4-8
 * @author Yang Tang
 * @version 1.0
 */
public class Screen4_8 extends LevelScreen {
    static {
        fileName = "maps/map_4/Map 4-8.tmx";
        thisScreenClass = "com.mygdx.screens.Screen4_8";
    }
    ...
    public void initialize() {
        ...
        for (MapObject obj : tma.getTileList("Mon62")) {
            MapProperties props = obj.getProperties();
            new Onix((float) props.get("x"), (float) props.get("y"), mainStage, "monster/round_4/monster_16/16-1.png");
        }

        for (MapObject obj : tma.getTileList("Mon63")) {
            MapProperties props = obj.getProperties();
            new Rhyhorn((float) props.get("x"), (float) props.get("y"), mainStage, "monster/round_4/monster_19/19-1.png");
        }

        for (MapObject obj : tma.getTileList("Mon64")) {
            MapProperties props = obj.getProperties();
            new Rhydon((float) props.get("x"), (float) props.get("y"), mainStage, "monster/round_4/monster_20/20-1.png");
        }
    ...
    public void update(float dt) {
        ...
        for (BaseActor monster : BaseActor.getList(mainStage, "com.mygdx.monster.round_4.Onix")) {
        ...
        for (BaseActor monster : BaseActor.getList(mainStage, "com.mygdx.monster.round_4.Rhyhorn")) {
            if (jack.overlaps(monster)) {
                ParadiseGame.backgroundMusic.pause();
                if (isDeathSoundPermitted) {
                    ParadiseGame.deathSound.play();
        ...
        for (BaseActor monster : BaseActor.getList(mainStage, "com.mygdx.monster.round_4.Rhydon")) {
            if (jack.overlaps(monster)) {
                ParadiseGame.backgroundMusic.pause();
                if (isDeathSoundPermitted) {
                    ParadiseGame.deathSound.play();
        ...
        for (BaseActor tip : BaseActor.getList(mainStage, "com.mygdx.maps.Tip1")) {
            if (jack.overlaps(tip) && Gdx.input.isKeyPressed(Input.Keys.Q)) {
                new TipActor(Tip1.x, Tip1.y, mainStage, "items/tips/map_4/tip12.png");
                tip.remove();
            }
        }
        ...
        for (BaseActor tip : BaseActor.getList(mainStage, "com.mygdx.maps.Tip2")) {
            if (jack.overlaps(tip) && Gdx.input.isKeyPressed(Input.Keys.Q)) {
                new TipActor(Tip2.x - 300, Tip2.y, mainStage, "items/tips/map_4/tip13.png");
                tip.remove();
            }
        }
