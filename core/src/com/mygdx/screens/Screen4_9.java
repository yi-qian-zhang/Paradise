...
import com.mygdx.monster.round_3.Foffing;
import com.mygdx.monster.round_4.Marowak;
import com.mygdx.monster.round_4.Nidoking;
import com.mygdx.monster.round_4.Nidoqueen;
...
/**
 * This Screen represents map 4-9
 * @author Yang Tang
 * @version 1.0
 */
public class Screen4_9 extends LevelScreen {
    static {
        fileName = "maps/map_4/Map 4-9.tmx";
        thisScreenClass = "com.mygdx.screens.Screen4_9";
    }
    ...
    public void initialize() {
        ...
        for (MapObject obj : tma.getTileList("Mon65")) {
            MapProperties props = obj.getProperties();
            new Marowak((float) props.get("x"), (float) props.get("y"), mainStage, new String[] {"monster/round_4/monster_18/18-1.png", "monster/round_4/monster_18/18-2.png"});
        }

        for (MapObject obj : tma.getTileList("Mon66")) {
            MapProperties props = obj.getProperties();
            new Nidoqueen((float) props.get("x"), (float) props.get("y"), mainStage, "monster/round_4/monster_6/6-1.png");
        }

        for (MapObject obj : tma.getTileList("Mon67")) {
            MapProperties props = obj.getProperties();
            new Nidoking((float) props.get("x"), (float) props.get("y"), mainStage, "monster/round_4/monster_7/7-1.png");
        }
        ...
        for (MapObject obj : tma.getTileList("Success")) {
            MapProperties props = obj.getProperties();
            new Success((float) props.get("x"), (float) props.get("y"), mainStage);
        }
    ...
    public void update(float dt) {
        ...
        for (BaseActor end1 : BaseActor.getList(mainStage, "com.mygdx.maps.End1")) {
            if (jack.overlaps(end1)) {
                start = "Start";
                ParadiseGame.setActiveScreen(new Screen3_8());
            }
        }
        ...
        for (BaseActor monster : BaseActor.getList(mainStage, "com.mygdx.monster.round_4.Marowak")) {
        ...
        for (BaseActor monster : BaseActor.getList(mainStage, "com.mygdx.monster.round_4.MarowakBone")) {
            if (jack.overlaps(monster)) {
                ParadiseGame.backgroundMusic.pause();
                if (isDeathSoundPermitted) {
                    ParadiseGame.deathSound.play();
        ...
        for (BaseActor monster : BaseActor.getList(mainStage, "com.mygdx.monster.round_4.Nidoqueen")) {
            if (jack.overlaps(monster)) {
                ParadiseGame.backgroundMusic.pause();
                if (isDeathSoundPermitted) {
                    ParadiseGame.deathSound.play();
        ...
        for (BaseActor monster : BaseActor.getList(mainStage, "com.mygdx.monster.round_4.Nidoking")) {
            if (jack.overlaps(monster)) {
                ParadiseGame.backgroundMusic.pause();
                if (isDeathSoundPermitted) {
                    ParadiseGame.deathSound.play();
        ...
        for (BaseActor tip : BaseActor.getList(mainStage, "com.mygdx.maps.Tip1")) {
            if (jack.overlaps(tip) && Gdx.input.isKeyPressed(Input.Keys.Q)) {
                new TipActor(Tip1.x, Tip1.y, mainStage, "items/tips/map_4/tip14.png");
                tip.remove();
            }
        }

        for (BaseActor success : BaseActor.getList(mainStage, "com.mygdx.maps.Success")) {
            if (jack.overlaps(success) && Gdx.input.isKeyPressed(Input.Keys.Q)) {
